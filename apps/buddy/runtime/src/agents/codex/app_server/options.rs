...
const CODEX_APP_SERVER_OPTIONS_DEADLINE: Duration = Duration::from_secs(25);
...
fn load_codex_model_options_with_program(program: &Path) -> BuddyResult<Vec<CodexModelOption>> {
    ...
    let _deadline = transport.spawn_deadline_watcher(CODEX_APP_SERVER_OPTIONS_DEADLINE);
