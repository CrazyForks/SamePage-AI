...
pub(super) struct CodexAppServerDeadlineWatcher {
    join_handle: Option<JoinHandle<()>>,
...
impl CodexAppServerTransport {
    ...
    pub(super) fn spawn_deadline_watcher(
        &self,
        timeout: Duration,
    ) -> CodexAppServerDeadlineWatcher {
        self.child.spawn_deadline_watcher(timeout)
    }
...
impl CodexAppServerProcessHandle {
    ...
    fn spawn_deadline_watcher(&self, timeout: Duration) -> CodexAppServerDeadlineWatcher {
        let process = self.clone();
...
impl Drop for CodexAppServerDeadlineWatcher {
    fn drop(&mut self) {
...
"#;
    ...
    #[test]
    #[cfg(unix)]
    fn deadline_watcher_terminates_app_server_child() {
        use std::{
            fs,
            os::unix::fs::PermissionsExt,
            time::{Duration, Instant},
        ...
        let temp_dir = std::env::temp_dir().join(format!(
            "lexora-buddy-codex-app-server-deadline-{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&temp_dir).expect("create temp dir");
        let script_path = temp_dir.join("codex-app-server");
...
"#;
        ...
        let _deadline = transport.spawn_deadline_watcher(Duration::from_millis(50));
        let started_at = Instant::now();
        let error = transport
            .read_line()
            .expect_err("expired app-server should close stdout");
