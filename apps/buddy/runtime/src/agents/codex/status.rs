...
pub fn detect_codex_runtime_status() -> CodexRuntimeStatus {
    let version_output = observe_probe("version", run_codex_success_probe(["--version"]));
    let login_status_output = if version_output.is_some() {
        observe_probe("login status", run_codex_output_probe(["login", "status"]))
    } else {
        None
    };
    let app_server_help_output = observe_probe(
        "app-server capability",
        run_codex_success_probe(["app-server", "--help"]),
    );
    let exec_help_output = observe_probe(
        "exec capability",
        run_codex_success_probe(["exec", "--help"]),
    );
...
fn observe_probe(
    name: &str,
    result: Result<Option<String>, super::probe::CodexProbeError>,
) -> Option<String> {
    match result {
        Ok(output) => output,
        Err(error) => {
            eprintln!("Codex {name} probe failed: {error}");
            None
        }
    }
}
