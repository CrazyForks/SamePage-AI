...
use super::{
    BuddyChatAttachment, TrustedCodexSkillInput, BUDDY_CHAT_IMAGE_MAX_BYTES,
    BUDDY_CHAT_IMAGE_MIME_TYPES,
};

const BUDDY_INLINE_IMAGE_MAX_BYTES: usize = BUDDY_CHAT_IMAGE_MAX_BYTES as usize;
...
pub(in crate::commands) fn compose_buddy_chat_codex_inputs(
    ...
    for input in requested_inputs {
        match input {
            ...
            codex::CodexUserInput::Image { detail, url } => {
                if is_safe_buddy_codex_image_url(&url) {
...
fn is_safe_buddy_codex_image_url(url: &str) -> bool {
    ...
    if !BUDDY_CHAT_IMAGE_MIME_TYPES.contains(&mime_type) {
        return false;
    }

    BASE64_STANDARD
        .decode(encoded)
        .is_ok_and(|bytes| bytes.len() <= BUDDY_INLINE_IMAGE_MAX_BYTES)
}
...
fn create_buddy_chat_image_input_url(
    ...
    if !BUDDY_CHAT_IMAGE_MIME_TYPES.contains(&attachment.mime_type.as_str())
        || attachment.size_bytes > BUDDY_CHAT_IMAGE_MAX_BYTES
    {
        return Ok(None);
    }
    ...
    if bytes.len() > BUDDY_INLINE_IMAGE_MAX_BYTES {
        return Ok(None);
    }
    Ok(Some(format!(
        "data:{};base64,{}",
