...
fn materializes_registered_text_attachment_from_storage() {
    ...
    let attachments = materialize_buddy_chat_attachments(
        &state,
        vec![BuddyChatAttachmentRequest { attachment_id }],
    )
...
#[test]
fn rejects_more_than_sixteen_attachments_per_turn() {
    let dir = std::env::temp_dir().join(format!("lexora-buddy-test-{}", uuid::Uuid::new_v4()));
    let state = BuddyAppState::initialize_with_paths(BuddyAppPaths::from_data_dir(dir.clone()))
        .expect("initialize state");
    let attachments = (0..17)
        .map(|index| BuddyChatAttachmentRequest {
            attachment_id: format!("attachment-{index}"),
        })
        .collect();

    let error = materialize_buddy_chat_attachments(&state, attachments)
        .expect_err("turn attachment count must be bounded");
...
#[test]
fn rejects_attachment_payloads_over_the_turn_byte_budget() {
    let dir = std::env::temp_dir().join(format!("lexora-buddy-test-{}", uuid::Uuid::new_v4()));
    let state = BuddyAppState::initialize_with_paths(BuddyAppPaths::from_data_dir(dir.clone()))
        .expect("initialize state");
    let path = dir.join("eight-megabytes.txt");
    fs::write(&path, vec![b'a'; 8 * 1024 * 1024]).expect("write attachment");
    let attachment = create_buddy_clipboard_file_from_path(&state, &path, "file-picker")
        .expect("register attachment")
        .expect("attachment");
    let attachment_id = attachment.attachment_id.expect("attachment id");
    let attachments = (0..5)
        .map(|_| BuddyChatAttachmentRequest {
            attachment_id: attachment_id.clone(),
        })
        .collect();

    let error = materialize_buddy_chat_attachments(&state, attachments)
        .expect_err("turn attachment bytes must be bounded");
...
#[test]
fn rejects_unregistered_attachment_fields_at_the_protocol_boundary() {
    let error = serde_json::from_value::<BuddyChatAttachmentRequest>(serde_json::json!({
        "attachmentId": "attachment-1",
        "previewPath": "/home/user/secret.png",
    }))
    .expect_err("renderer paths must not cross the attachment boundary");
...
#[test]
fn rejects_malformed_and_unsupported_inline_image_inputs() {
    let input = compose_buddy_chat_codex_inputs(
        "看图",
        vec![
            codex::CodexUserInput::Image {
                detail: None,
                url: "data:image/svg+xml;base64,PHN2Zz4=".to_owned(),
            },
            codex::CodexUserInput::Image {
                detail: None,
                url: "data:image/png;base64,not-base64".to_owned(),
            },
        ],
        &[],
        None,
    ...
    assert!(!input
        .iter()
        .any(|item| matches!(item, codex::CodexUserInput::Image { .. })));
}
