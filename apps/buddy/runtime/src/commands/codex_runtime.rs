...
use crate::{
    ...
    state::BuddyAppState,
    storage::{BuddyRun, BuddyStorage},
};

#[cfg(test)]
use crate::storage::{BuddyMessage, BuddyRunEvent};

use super::{
    host_action::{append_buddy_host_action_events, execute_buddy_host_actions},
...
use terminal_state::{claim_completion_or_abort_cancelled_run, record_codex_run_failure};

pub(super) struct BuddyCodexExecution {
    #[cfg(test)]
...
pub(super) struct ExistingCodexRunRequest {
    ...
    pub(super) host_action_executor: Option<NativePetChoreographyStepExecutor>,
    pub(super) host_action_state: BuddyAppState,
    pub(super) user_message_id: String,
}

pub(super) fn execute_existing_codex_run(
    request: ExistingCodexRunRequest,
) -> Result<BuddyCodexExecution, BuddyError> {
    let ExistingCodexRunRequest {
        approval_id,
        attachment_count,
        runtime_cwd,
        content,
    ...
    claim_completion_or_abort_cancelled_run(
        &storage,
        &run.id,
        approval_id,
    ...
    let host_actions = append_buddy_host_action_events(
        &storage,
        &run.id,
        &mut events,
    ...
    if let Some(executor) = host_action_executor {
        if let Err(error) =
            execute_buddy_host_actions(&host_action_state, &run.id, host_actions, &executor)
        {
            eprintln!("lexora buddy host action execution failed: {error}");
        }
    }
