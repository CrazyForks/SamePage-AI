...
pub(crate) fn list_conversation_attachment_ids(
    connection: &Connection,
    conversation_id: &str,
) -> BuddyResult<Vec<String>> {
    let mut statement = connection.prepare(
        r#"
        SELECT DISTINCT json_extract(message_attachment.value, '$.attachmentId')
        FROM messages, json_each(messages.attachments_json) AS message_attachment
        WHERE messages.conversation_id = ?1
          AND json_type(message_attachment.value, '$.attachmentId') = 'text'
        ORDER BY 1
        "#,
    )?;
