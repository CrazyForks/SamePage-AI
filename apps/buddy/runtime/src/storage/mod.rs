...
use crate::{
    app_paths::ATTACHMENTS_DIR_NAME,
    domain::{
...
pub(crate) use attachments::remove_attachment_file;
...
pub struct BuddyStorage {
    ...
    agent_turn_request_coordinator: Arc<Mutex<()>>,
...
impl BuddyStorage {
    ...
    pub fn delete_conversation(&self, id: String) -> BuddyResult<bool> {
        let Some((conversation, runs, attachment_ids)) =
            self.with_connection("prepare_delete_conversation", |connection| {
                let Some(conversation) =
                    conversations::find_optional_conversation(connection, &id)?
                else {
                    return Ok(None);
                };
                let runs = runs::list_conversation_run_log_refs(connection, &id)?;
                let attachment_ids = messages::list_conversation_attachment_ids(connection, &id)?;

                Ok(Some((conversation, runs, attachment_ids)))
            })?
        else {
            return Ok(false);
        };
        ...
        for run in &runs {
            self.local_logs
                .append_run_deleted_index_entry(&run.id, &run.log_path)?;
        }

        let deleted = self.with_connection("delete_conversation", |connection| {
        ...
        for run in runs {
            self.local_logs.remove_log_file(&run.log_path)?;
        }
        self.local_logs.remove_log_file(&conversation.log_path)?;
        let attachments_dir = self.local_logs.absolute_path(ATTACHMENTS_DIR_NAME);
        for attachment_id in attachment_ids {
            let Some(_released) =
                self.release_unreferenced_attachment_file(&attachments_dir, &attachment_id)?
            else {
                continue;
            };
        }
