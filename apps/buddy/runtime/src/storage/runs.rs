...
#[derive(Debug)]
pub(crate) struct BuddyRunLogRef {
    pub(crate) id: String,
...
impl BuddyStorage {
    ...
    pub fn list_conversation_runs(
        &self,
        conversation_id: String,
        limit: i64,
    ) -> BuddyResult<Vec<BuddyRun>> {
        self.with_connection("list_conversation_runs", |connection| {
            self::list_conversation_runs(connection, conversation_id, limit)
        })
    }
...
pub(super) fn list_non_terminal_run_ids(connection: &Connection) -> BuddyResult<Vec<String>> {
    let mut statement = connection.prepare(
        r#"
        SELECT id
        FROM runs
...
fn list_conversation_runs(
    ...
    let runs = statement
        .query_map(params![conversation_id, limit], map_run)?
        .collect::<Result<Vec<_>, _>>()?;

    Ok(runs)
}

pub(crate) fn list_conversation_run_log_refs(
    connection: &Connection,
    conversation_id: &str,
) -> BuddyResult<Vec<BuddyRunLogRef>> {
    let mut statement = connection.prepare(
        r#"
        SELECT id, log_path
        FROM runs
...
mod contract_tests {
    ...
    #[test]
    fn serializes_the_desktop_local_chat_run_fixture() {
        let run = BuddyRun {
            id: "run-contract-1".to_owned(),
            session_id: None,
            conversation_id: Some("conversation-contract-1".to_owned()),
            branch_id: Some("branch-contract-1".to_owned()),
            triggering_message_id: Some("message-contract-1".to_owned()),
            intent: Some("buddy.agent.turn".to_owned()),
            log_path: Some("/tmp/run-contract.jsonl".to_owned()),
        ...
        let fixture = serde_json::from_str::<serde_json::Value>(include_str!(
            "../../../protocol-fixtures/local-chat-run.json"
        ))
        .expect("parse desktop run fixture");

        assert_eq!(serde_json::to_value(run).expect("serialize run"), fixture);
    }
}
