...
use super::affective::{AffectiveContext, AffectiveEnergy, AffectiveMood, ResolveContext};
...
pub(crate) const STEP_FALLBACK_RESOLUTION_REASON_CODE: &str = "fallback.stepActionResolved";
pub(crate) const AFTER_ACTION_FALLBACK_RESOLUTION_REASON_CODE: &str =
...
impl StepResolution {
    pub(crate) fn with_step_fallback(
        mut self,
        ...
        self.fallback = Some(StepResolutionFallback {
            ...
            reason_code: STEP_FALLBACK_RESOLUTION_REASON_CODE.to_owned(),
...
impl ActionRegistry {
    ...
    pub(crate) fn resolve_action_for_animation(
        &self,
        action_selector: &str,
        animation_ref: &str,
        context: &ResolveContext,
        ...
        let action = self
            .select_animation_candidate(action_selector, animation_ref, context.affective_context)
            .ok_or_else(|| {
                BuddyError::Validation(format!(
                    "unknown buddy action selector or animationRef: {action_selector} -> {animation_ref}"
                ))
            })?;

        self.resolve_action_entry(action)
    }
    ...
    fn select_animation_candidate(
        ...
        self.actions
            .values()
            .filter(|action| {
                action.animation_ref == animation_ref
                    && action_matches_selector(action, action_selector)
            })
            .min_by(|left, right| {
                selector_sort_key(left, affective_context)
                    .cmp(&selector_sort_key(right, affective_context))
...
mod tests {
    ...
    use crate::choreography::affective::{
