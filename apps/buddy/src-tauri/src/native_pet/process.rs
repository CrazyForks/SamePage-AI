...
use super::{
    animation::{NativePetAnimationKey, NativePetAnimationSet},
...
pub(super) use control_protocol::{
    compile_execute_step_control_message, parse_native_pet_control_message,
    parse_native_pet_control_request_kind, parse_native_pet_stdin_control_request_kind,
    NativePetAnchorReveal, NativePetControlMessage, NativePetControlRequestKind, NativePetWalkEdge,
    NativePetWalkTarget, NativePetWindowAnchorEdge, NativePetWindowAnchorReveal,
...
impl NativePetSidecarConnection {
    ...
    fn set_animation(&self, animation: &str) -> BuddyResult<()> {
        let animation = NativePetAnimationKey::parse(animation).ok_or_else(|| {
            BuddyError::Runtime(format!("invalid native pet animation key: {animation}"))
        })?;
    ...
    fn send_control_message(&self, message: NativePetControlMessage) -> BuddyResult<()> {
        self.send_control_line(&format_native_pet_control_message(message))
    }
...
impl NativePetSidecarProcess {
    ...
    pub fn set_animation(&self, animation: &str) -> BuddyResult<()> {
        self.current_connection()?.set_animation(animation)
    }
}
        ...
        for line in stdin.lock().lines().map_while(Result::ok) {
            ...
            let request = match kind {
                ...
                | NativePetControlRequestKind::InterruptStep(_) => {
                    NativePetControlRequest::step(kind)
                }
                NativePetControlRequestKind::QueryState
                | NativePetControlRequestKind::QueryCapabilities => continue,
            };
            if sender.send(request).is_err() {
                break;
            }
...
mod tests {
    use super::{
        ...
        native_pet_step_response_matches_step_id, parse_native_pet_sidecar_event,
        receive_native_pet_sidecar_ready, receive_native_pet_step_response,
        spawn_native_pet_sidecar_with_startup_retry, NativePetControlMessage, NativePetControlPoll,
