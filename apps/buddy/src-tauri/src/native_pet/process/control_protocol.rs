...
impl NativePetWindowAnchorEdge {
    ...
    pub(in crate::native_pet) fn key(self) -> &'static str {
        match self {
            Self::Auto => "auto",
            Self::Left => "left",
            Self::Right => "right",
    ...
    match message {
        ...
        NativePetControlMessage::WalkByPath { path, after } => {
            let mut request = serde_json::json!({
                ...
                "path": path
                    .into_iter()
                    .map(format_native_pet_walk_target_json)
                    .collect::<Vec<_>>(),
            });
            if let Some(after) = after {
                request["after"] = serde_json::Value::String(after.manifest_key().to_owned());
...
fn format_native_pet_walk_target_json(target: NativePetWalkTarget) -> serde_json::Value {
    match target {
        ...
        NativePetWalkTarget::EdgeAnchor {
            ...
            "kind": "edgeAnchor",
            "edge": edge.key(),
            "reveal": format_native_pet_anchor_reveal_key(reveal),
        ...
        NativePetWalkTarget::WindowAnchor {
            ...
            "kind": "windowAnchor",
            "selector": format_native_pet_window_anchor_selector_json(selector),
            "edge": edge.key(),
            "reveal": format_native_pet_anchor_reveal_key(reveal),
            "durationMs": duration_ms,
        }),
...
fn format_native_pet_window_anchor_selector_json(
    selector: NativePetWindowAnchorSelector,
) -> serde_json::Value {
    match selector.kind {
        NativePetWindowAnchorSelectorKind::ActiveWindow => {
            serde_json::json!({ "kind": "activeWindow" })
        }
    }
}

fn format_native_pet_anchor_reveal_key(reveal: NativePetAnchorReveal) -> &'static str {
    match reveal {
        NativePetAnchorReveal::Head => "head",
    }
}

fn format_native_pet_after_suffix(after: Option<NativePetAnimationKey>) -> String {
    after
